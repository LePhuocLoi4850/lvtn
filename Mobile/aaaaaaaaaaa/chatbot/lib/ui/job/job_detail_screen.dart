...
class _BottomButtonState extends State<BottomButton> {
  bool isApplied =
      false; // Dùng để lưu trữ trạng thái ứng tuyển của công việc này

  @override
  void initState() {
    super.initState();
    _checkIsApplied(); // Kiểm tra và cập nhật trạng thái ứng tuyển ban đầu khi widget được tạo ra
  }

  void _checkIsApplied() async {
    final appliedJobs = await JobPreferences.getAppliedJobs();
    setState(() {
      isApplied = appliedJobs.contains(widget.job.id);
    });
  }
        ...
        child: Row(
          ...
          children: <Widget>[
            Container(
              ...
              child: ElevatedButton(
                style: ElevatedButton.styleFrom(
                  backgroundColor: isApplied ? Colors.red : Colors.blue,
                  shape: RoundedRectangleBorder(
                ...
                onPressed: () async {
                  ...
                  if (!isApplied) {
                    ...
                    await JobPreferences.saveJobApplied(widget.job.id!, true);
                  } else {
                    cart.clearItem(widget.job.id!);
                    notification.clearNotificationItem(widget.job.id!);
                    await JobPreferences.saveJobApplied(widget.job.id!, false);
