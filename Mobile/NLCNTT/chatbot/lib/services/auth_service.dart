...
class AuthService {
  ...
  Future<AuthToken> getUserAuthToken(UserData userData) async {
  ...
  // }
  String isauth() {
    return _newUser != null ? _newUser!.level : "user";
  }
  ...
  // }
  Future<UserData> login(String email, String password, String role) async {
    try {
      // Thực hiện đăng nhập và nhận authentication token
      final authToken =
          await _authenticate(email, password, role, 'signInWithPassword');
      ...
      if (authToken != null && authToken.isValid) {
        ...
        if (userData != null) {
          ...
          // Nếu không tìm thấy thông tin người dùng, ném ra một exception
