...
class InsertEducation extends StatefulWidget {
  const InsertEducation({super.key});

  @override
  State<InsertEducation> createState() => _InsertEducationState();
}

class _InsertEducationState extends State<InsertEducation> {
  ...
  Future<void> _handleInsertEducation() async {
    setState(() {
      isLoading = true;
    });
    int uid = controller.userModel.value.id!;
    String level = _educationController.text;
    ...
    try {
      await Database().insertEducation(
          uid, level, name, timeFrom, timeTo, description, career);
      setState(() {
        isLoading = false;
      });
                                ...
                                children: [
                                  Text(
                                    _educationController.text.isEmpty ||
                                        ...
                                        ? 'Chọn cấp bậc'
                                        : _educationController.text,
                                    style: _educationController.text.isEmpty ||
                                            _educationController.text ==
                                                'Không yêu cầu'
                                        ? TextStyle(
                                            fontSize: 18,
